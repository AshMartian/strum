...
def test_explicitly_empty_runtime_revision_rejects_pinned_bundle(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    write_manifest(
        tmp_path,
    ...
    with pytest.raises(BundleValidationError, match="source revision configuration is invalid"):
        load_model_bundle(tmp_path)
